    ...
    public void simulate(Scheduler scheduler, Queue<Process> processes) {
        ...
        while (!done) {
            ...
            for (int cpu = 0; cpu < this.cpuCount; cpu++) {
                ...
                // scheduler.update() is passed the previously running process, where it is 1) run more or 2) put back in the queue
                // that scheduler.update() call passes back the process that should be run next.
